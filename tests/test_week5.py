...
class TestBettingSystemValidation:
...
class TestBlackjackPayout:
    def test_natural_payout(self):
        assert BettingSystem.calculate_payout(100, "natural", "blackjack") == 250

    def test_win_payout(self):
        assert BettingSystem.calculate_payout(100, "win", "blackjack") == 200

    def test_push_returns_bet(self):
        assert BettingSystem.calculate_payout(100, "push", "blackjack") == 100

    def test_bust_payout_is_zero(self):
        assert BettingSystem.calculate_payout(100, "bust", "blackjack") == 0

    def test_lose_payout_is_zero(self):
        assert BettingSystem.calculate_payout(100, "lose", "blackjack") == 0


class TestBaccaratPayout:
    def test_punto_win_punto_bet(self):
        assert BettingSystem.calculate_payout(100, "punto_win", "baccarat", "punto") == 200

    def test_banco_win_banco_bet(self):
        payout = BettingSystem.calculate_payout(100, "banco_win", "baccarat", "banco")
        assert payout == 195   # 5% commission

    def test_tie_tie_bet(self):
        assert BettingSystem.calculate_payout(100, "tie", "baccarat", "tie") == 900

    def test_tie_punto_bet_is_push(self):
        assert BettingSystem.calculate_payout(100, "tie", "baccarat", "punto") == 100

    def test_tie_banco_bet_is_push(self):
        assert BettingSystem.calculate_payout(100, "tie", "baccarat", "banco") == 100

    def test_punto_win_banco_bet_loses(self):
        assert BettingSystem.calculate_payout(100, "punto_win", "baccarat", "banco") == 0

    def test_banco_win_punto_bet_loses(self):
        assert BettingSystem.calculate_payout(100, "banco_win", "baccarat", "punto") == 0
...
class TestBaccaratNatural:
    ...
    def test_natural_9_no_third_card(self):
        ...
        assert game.natural is True
        assert game.phase == "done"
        assert game.punto_total() == 9
    ...
    def test_outcome_punto_wins(self):
        ...
        assert result["outcome"] == "punto_win"
...
class TestBlackjackGame:
    ...
    def test_start_round_deducts_bet(self):
        ...
        # Just need a valid 4-card start + extras for dealer
        cards = [
            Card("Hearts", "5"), Card("Clubs", "2"),
            Card("Hearts", "6"), Card("Clubs", "3"),
            Card("Spades", "9"), Card("Diamonds", "5"),  # dealer hits to >= 17
        ]
        game.start_round(100, deck=self._make_deck(cards))
        assert game.player.bankroll == 400   # 500 - 100

    def test_natural_blackjack_phase_done(self):
        # Deal: player A, dealer 2, player K, dealer 3 → player=21 natural
        # Dealer (total 5) then hits: 5+2+3 = 10, hits more...
        # Let's give dealer enough cards to finish
        cards = [
            Card("Spades", "A"), Card("Clubs", "2"),
            Card("Hearts", "K"), Card("Diamonds", "3"),
            Card("Hearts", "5"), Card("Clubs", "9"),   # dealer hits: 5+2+3+5=15, 15+9=24>17? no wait
            # Dealer has 2+3=5, needs to hit. Give: 5 (total=10), hit again: 9 (total=19 ≥17)
        ]
        player = Player("Tester", bankroll=500)
        game = BlackjackGame(player)
        game.start_round(100, deck=self._make_deck(cards))
    ...
    def test_hit_adds_card(self):
        ...
        game = BlackjackGame(player)
        game.start_round(100, deck=self._make_deck(cards))
        assert game.phase == "player"
        assert game.current_hand.card_count() == 2
        game.hit()
        assert game.current_hand.card_count() == 3
    ...
    def test_settle_bets_credits_win(self):
        ...
        game.stand()
        game.settle_bets()
        assert game.player.bankroll == 600  # 400 (after bet) + 200 payout
